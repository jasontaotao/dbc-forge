...
describe('signal-name-format', () => {
  it('passes for valid signal name', () => {
    ...
    net = addMessage(net, createMessage({
      ...
      signals: [createSignal({
        name: 'Speed_kmh', startBit: 0, length: 8, byteOrder: 'little-endian',
        valueType: 'unsigned', factor: 1, offset: 0, min: 0, max: 255, unit: 'km/h', receivers: [],
      })],
    }));
  ...
  it('fires for invalid name', () => {
    ...
    net = addMessage(net, createMessage({
      id: 1, name: 'M', dlc: 8, transmitter: 'BCM',
      signals: [createSignal({
        name: '1Speed', startBit: 0, length: 8, byteOrder: 'little-endian',
