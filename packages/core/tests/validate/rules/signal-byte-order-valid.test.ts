  ...
  it('passes for little-endian and big-endian', () => {
    ...
    net = addMessage(net, createMessage({
      ...
      signals: [
        ...
        createSignal({ name: 'B', startBit: 8, length: 8, byteOrder: 'big-endian', valueType: 'unsigned', factor: 1, offset: 0, min: 0, max: 255, unit: '', receivers: [] }),
      ],
    }));
  ...
  it('never fires (type-system enforced)', () => {
    ...
    net = addMessage(net, createMessage({
      ...
      signals: [createSignal({
        name: 'A', startBit: 0, length: 8, byteOrder: 'little-endian',
