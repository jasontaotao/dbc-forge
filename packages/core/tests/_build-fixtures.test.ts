...
function buildFullAttributes(): Network {
  ...
  net = addAttributeDef(net, {
    name: 'Baudrate',
    target: 'network',
    type: { kind: 'int', min: 0, max: 1_000_000 },
    defaultValue: 500000,
  });
  ...
  for (const attr of [
    {
      ...
      type: { kind: 'int' as const, min: 0, max: 65535 },
      defaultValue: 100,
    },
    {
      name: 'GenMsgSendType',
    ...
    net = addAttributeAssignment(net, {
      name: attr.name,
      target: { kind: 'message', messageId: 0x300 },
  ...
  for (const attr of [
    {
      ...
      type: { kind: 'float' as const, min: -1e9, max: 1e9 },
      defaultValue: 0,
    },
    {
