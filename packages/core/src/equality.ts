...
function stableRepr(net: Network): unknown {
  return {
    ...
    messages: [...net.messages].sort((x, y) => x.id - y.id).map(stabilizeMessage),
    valueTables: dedupeValueTables(net.valueTables),
    signalGroups: [...net.signalGroups].sort((x, y) =>
      `${x.messageId}-${x.name}`.localeCompare(`${y.messageId}-${y.name}`),
    ),
 ...
 *  absent). */
function stabilizeMessage(m: Network['messages'][number]): unknown {
...
function dedupeValueTables(
  vts: ReadonlyArray<{ name: string; entries: ReadonlyArray<{ raw: number; name: string }> }>,
): Array<{ name: string; entries: ReadonlyArray<{ raw: number; name: string }> }> {
