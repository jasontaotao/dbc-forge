...
describe('mux-switch-required-when-mux', () => {
  ...
  it('fires when Muxed signal exists without a Multiplexor', () => {
    ...
    net = addMessage(net, createMessage({
      id: 1, name: 'M', dlc: 8, transmitter: 'BCM',
      signals: [
        createSignal({ name: 'A0', startBit: 0, length: 8, byteOrder: 'little-endian', valueType: 'unsigned', factor: 1, offset: 0, min: 0, max: 1, unit: '', receivers: [], multiplexed: { kind: 'Muxed', value: 0 } }),
      ],
    }));
