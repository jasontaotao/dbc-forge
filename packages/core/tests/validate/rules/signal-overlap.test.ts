...
describe('signal-overlap', () => {
  ...
  it('fires when two plain signals overlap', () => {
    ...
    net = addMessage(net, createMessage({
      ...
      signals: [
        createSignal({ name: 'A', startBit: 0, length: 8, byteOrder: 'little-endian', valueType: 'unsigned', factor: 1, offset: 0, min: 0, max: 255, unit: '', receivers: [] }),
        createSignal({ name: 'B', startBit: 4, length: 8, byteOrder: 'little-endian', valueType: 'unsigned', factor: 1, offset: 0, min: 0, max: 255, unit: '', receivers: [] }),
      ],
    }));
  ...
  it('passes when muxed signals in different buckets overlap', () => {
    ...
    net = addMessage(net, createMessage({
      ...
      signals: [
        ...
        createSignal({ name: 'A0', startBit: 8, length: 16, byteOrder: 'little-endian', valueType: 'unsigned', factor: 1, offset: 0, min: 0, max: 1, unit: '', receivers: [], multiplexed: { kind: 'Muxed', value: 0 } }),
        createSignal({ name: 'A1', startBit: 8, length: 16, byteOrder: 'little-endian', valueType: 'unsigned', factor: 1, offset: 0, min: 0, max: 1, unit: '', receivers: [], multiplexed: { kind: 'Muxed', value: 1 } }),
      ],
    }));
    expect(signalOverlap.check(net)).toHaveLength(0);
  });
  ...
  it('fires when muxed signals in same bucket overlap', () => {
    ...
    net = addMessage(net, createMessage({
      ...
      signals: [
        createSignal({ name: 'Mux', startBit: 0, length: 8, byteOrder: 'little-endian', valueType: 'unsigned', factor: 1, offset: 0, min: 0, max: 255, unit: '', receivers: [], multiplexed: 'Multiplexor' }),
        createSignal({ name: 'A0', startBit: 8, length: 16, byteOrder: 'little-endian', valueType: 'unsigned', factor: 1, offset: 0, min: 0, max: 1, unit: '', receivers: [], multiplexed: { kind: 'Muxed', value: 0 } }),
        createSignal({ name: 'B0', startBit: 16, length: 8, byteOrder: 'little-endian', valueType: 'unsigned', factor: 1, offset: 0, min: 0, max: 1, unit: '', receivers: [], multiplexed: { kind: 'Muxed', value: 0 } }),
      ],
    }));
