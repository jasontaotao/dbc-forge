...
describe('attr-rel-target-exists', () => {
  it('passes when relation target exists', () => {
    ...
    net = addMessage(net, createMessage({
      ...
      signals: [createSignal({
        ...
        valueType: 'unsigned', factor: 1, offset: 0, min: 0, max: 1, unit: '', receivers: ['GW'],
      })],
    }));
    net = addRelationAttributeDef(net, { name: 'GenSigTimeoutTime', target: 'node-signal', type: { kind: 'int', min: 0, max: 65535 }, defaultValue: 0 });
    net = addRelationAttributeAssignment(net, { name: 'GenSigTimeoutTime', target: { kind: 'node-signal', nodeName: 'GW', messageId: 1, signalName: 'S' }, value: 100 });
  ...
  it('fires when relation target points at nonexistent message', () => {
    let net = createNetwork({ version: '1.0' });
    net = addRelationAttributeDef(net, { name: 'GenSigTimeoutTime', target: 'node-signal', type: { kind: 'int', min: 0, max: 65535 }, defaultValue: 0 });
    net = addRelationAttributeAssignment(net, { name: 'GenSigTimeoutTime', target: { kind: 'node-signal', nodeName: 'GW', messageId: 999, signalName: 'S' }, value: 100 });
